...
class Connect4 {
	...
	newGame(msg) {
		...
		const embed = new Discord.MessageEmbed()
			.setColor(crimson)
			.setTitle('Connect-4 • Discord Edition')
	...
	waitForReaction() {
		this.gameEmbed
			...
			.then((collected) => {
				...
				reaction.users
					.remove(reaction.users.cache.filter((user) => user.id !== this.gameEmbed.author.id).first().id)
					.then(() => {
						if (placedY == 0) this.gameEmbed.reactions.cache.get(reaction.emoji.name).remove();
	...
	isBoardFull() {
		for (let y = 0; y < HEIGHT; y++)
			for (let x = 0; x < WIDTH; x++) if (gameBoard[y * WIDTH + x] === '⚪') return false;
		return true;
	}

	getWinnerText(winner) {
		if (winner === '🔴' || winner === '🟡') return '🎉 • ' + winner + ' *won!* • 🎉';
		else if (winner == 'tie') return '*It was a tie!*';
		else if (winner == 'timeout') return '*Game timed out, no one responded in 5 minutes.*';
	}
}
